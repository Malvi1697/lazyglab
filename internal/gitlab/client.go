...
type Client struct {
	...
	// token and baseURL are kept for the one thing client-go cannot do: the GraphQL query
	// in graphql.go.
	token   string
	baseURL string
	...
	// titleCache remembers a commit's title by SHA.
	titleCache sync.Map // SHA (string) -> title (string)

	// stageCache remembers the stages of a finished pipeline, which are as immutable as
	// its verdict.
...
func ValidateToken(baseURL, token string, httpClient ...*http.Client) (string, error) {
	// Default client has no timeout, which would hang setup forever against a black-holed
	// host.
