...
// A failed load has to reach the status bar.
...
func TestLoadErrors_TheCommitPageIsNotSilentEither(t *testing.T) {
	// The page returned its message as a string, and Overview — the default view — dropped
	// it on the floor, so nothing on the commit page could report anything.
	v := NewDashboardView(&Context{})
	v.items = []gitlab.Commit{{ID: "abc1234", ShortID: "abc1234", Title: "one"}}
