...
func TestCommitRows_ShowAuthorNotHash(t *testing.T) {
	...
	row := commitItemRow(v.visible()[0], 6, 6, 60, 120)
...
func TestOverviewRows_ShowAuthorNotHash(t *testing.T) {
	...
	row := v.commitRow(v.visible()[0], 6, 6, 60, 120)
...
func TestCommitRows_LongAuthorIsTruncatedForAlignment(t *testing.T) {
	...
	visible := v.visible()
	rows := []string{commitItemRow(visible[0], 6, 6, 60, 120), commitItemRow(visible[1], 6, 6, 60, 120)}
