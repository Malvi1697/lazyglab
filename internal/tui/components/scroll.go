...
// ScrollMargin is how many rows stay visible past the cursor before a list starts
// scrolling — vim's scrolloff.
const ScrollMargin = 3

// ScrollOffset returns the index of the first visible row for a list of total items
// shown in a window of height rows, given the previous offset and the cursor's
// position.
func ScrollOffset(offset, cursor, total, height int) int {
	...
	// A window too short for margins on both sides keeps the cursor centred-ish instead of
	// refusing to scroll.
	margin := ScrollMargin
